...
public final class EvaluateConcordanceOnNonGenotypedSVVCFs extends AbstractIntervalTreeBasedConcordanceWalker {
    ...
    protected Predicate<VariantContext> makeEvalVariantFilter() {
        return vc -> {
            ...
            if (svType.equals(SimpleSVType.TYPES.DEL.name())) { // some tiny deletion calls are actually RPL calls with micro deletions
                if ( (!vc.hasAttribute(GATKSVVCFConstants.INSERTED_SEQUENCE)) && svLen < StructuralVariationDiscoveryArgumentCollection.STRUCTURAL_VARIANT_SIZE_LOWER_BOUND) // no ins_seq AND tiny
                    return false;
            } else if (svType.equals(SimpleSVType.TYPES.INS.name()) || svType.equals(SimpleSVType.TYPES.DUP.name())) {
    ...
    protected float computeConcordanceScore(final VariantContext eval, final VariantContext truth, final ReferenceContext context) {
        ...
        if (evalType.equals(SimpleSVType.TYPES.INS.name()) || evalType.equals(SimpleSVType.TYPES.DUP.name())) {
            if (truthType.equals(SimpleSVType.TYPES.INS.name())) { // truth type doesn't call dup, instead type them as ins
                ...
                return 0f;
        } else if (evalType.equals(SimpleSVType.TYPES.DEL.name())){
            if (truthType.equals(SimpleSVType.TYPES.DEL.name())) { // NOTE: one can use svlen, but remember the truth set might report svlen as positive for del
            ...
            } else if (truthType.equals(SimpleSVType.TYPES.INS.name()) && eval.hasAttribute(GATKSVVCFConstants.INSERTED_SEQUENCE_LENGTH)) { // some call set might report replacement as deletion with insertion
                ...
                return 0f;
        } else if (evalType.equals(SimpleSVType.TYPES.INV.name())) {
